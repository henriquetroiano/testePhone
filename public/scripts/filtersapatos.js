...
export default function selSapato() {
  const cadaSapato = document.querySelectorAll('li')
  cadaSapato.forEach((li) => {

    if (li.innerHTML.startsWith('SAPATOS')) {
  ...
  $.ajax({
    ...
    success: function (data) {
      $(data.items).each(function (index, value) {
        ...
        produtoIndividual.classList.add('sapatos');
